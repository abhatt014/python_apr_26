...
@app.route('/submit') # This route handles the GET request from the first form
def handle_submission_get():
    # 'user_name' matches the 'name' attribute of the input field in the HTML form
    name_from_form = request.args.get('user_name')
    if name_from_form:
        return f"<h1>Hello, {name_from_form}! (Submitted via GET)</h1>"
    else:
